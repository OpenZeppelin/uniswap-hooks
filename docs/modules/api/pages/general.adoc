...
NOTE: Only what a swap gained is taken. A swap filled worse than the recorded price keeps that fill and is
paid nothing back, so liquidity withdrawn after the block's first swap costs a swapper without the hook
intervening.
...
The beginning-of-block price values the swap past what a `BalanceDelta` carries, so the amount it

...
Returns the checkpoint `poolId` holds, which is zero for a pool never swapped in.
...
The swap would owe more than a `BalanceDelta` holds, so the amount cannot be charged.

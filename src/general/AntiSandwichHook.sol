 ...
 * hook fee. A sandwich therefore closes no better than the price its opening leg moved away from, and ends
 * never ends the block ahead. Only the price is read, so neither the tick layout nor the depth the pool holds
 * enters the target.
 *
 ...
 * held to the opening price. Whoever moved it is held to the same price squaring its own position.
 *
 * IMPORTANT: Only a swap's gain is taken, and its loss is never made good. A swap filled worse than the
 * recorded price keeps that fill and is paid nothing, so liquidity withdrawn after the block's first swap
 * costs the swapper and the hook does not make up the difference. The protection is one-sided in that sense:
 * it removes the attacker's profit rather than restoring the victim's.
 *
 * IMPORTANT: Only swaps are bounded. An attacker can supply the liquidity a victim trades through and
 ...
 *
 * IMPORTANT: A swap is refused with {TargetOutOfRange} where the recorded price values it past what a
 * `BalanceDelta` carries, rather than let through uncharged. Reaching it needs the price to move by an
 * enormous factor inside one block: the measured case crosses about 800,000 ticks, close to half of what a
 * pool can express. That in turn needs liquidity thin enough for one swap to cross that range, so a pool
 * holding its liquidity near its price cannot reach it, while a single position stretched over most of the
 * tick range can. Consider the width liquidity is provided over before deploying.
 *
 ...
 * later one. See {AntiSandwichMock}.
 *
 * WARNING: Since this hook removes the profit from sandwiching, there is less arbitrage in the pool, so
 * prices at the beginning of a block are not necessarily close to market price.
 *
...
abstract contract AntiSandwichHook is BaseDynamicAfterFee {
    ...
    /// @dev The beginning-of-block price values the swap past what a `BalanceDelta` carries, so the amount it
    /// would have to pay cannot be charged.
     ...
     * moves within a block re-records the price on every swap, which removes the protection. One that never moves
     * anchors the pool to a single price. Both fail silently.
     */

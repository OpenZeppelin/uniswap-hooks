...
abstract contract LimitOrderHook is BaseHook, IUnlockCallback {
    ...
    struct OrderInfo {
        ...
        /// @dev Monotonic fee accumulators that stores the
        // accumulated fees generated by the order per liquidity unit.
    ...
    struct UserInfo {
        ...
        /// Used to compute the portion of the accumulated fees that are owed to the owner.
    ...
    /// @dev Tracks each order id for a given `orderKey`, defined by {_orderKey}
        ...
        onlyValidPools(key.hooks)
    {
        ...
        // Account accrued fees after placement callback before updating the order liquidity and re-baselining the user fee checkpoints.
     ...
     * and redistributed to remaining liquidity providers. If the caller removes the last liquidity of the order, anny
     * accumulated accrued fees are paid out to `to` and the order id is reset to the default value.
     */
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        if (isLastWithdrawer) {
            // Sweep all remaining accounting dust to the final claimant.
            ...
            // Standard pro-rata path (rounding down).
        ...
        returns (uint256 amount0Fee, uint256 amount1Fee)
    {
        ...
        // If not all liquidity is removed, any accrued fees in the position are taken and stored in the hook.
    ...
    function getExtendedtOrderInfo(OrderIdLibrary.OrderId orderId)
        public
        view
        returns (
            bool filled,

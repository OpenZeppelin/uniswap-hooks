 ...
 *
 * The LimitOrderHook uses OrderIds to track distinct lifecycle instances of orders placed at the same pool locations.
 *
 ...
 * cancelled, the position is removed from the pool, the ID is reset to the default value and users withdraw their liquidity.
 *
 * If users later place a new order with the same order parameters, the OrderKey remains identical but maps
 ...
 * Therefore, an OrderKey may map to different OrderIds over time as orders are filled and recreated.
 */
library OrderIdLibrary {
    ...
    /// @dev Increment the order id. Might overflow.
    function unsafeIncrement(OrderId id) internal pure returns (OrderId) {
        unchecked {
            return OrderId.wrap(OrderId.unwrap(id) + 1);
        }
    }
}
 ...
 *
 * Note that given the way UniswapV4 pools works, when liquidity is placed out of range a single currency
 * is provided, instead of both currencies as in in-range liquidity additions.
 *
 * Orders can be cancelled at any time until they are filled and their liquidity is removed from the pool
 * into the hook, waiting for liquidity withdrawals from the order placers.
 *
 * IMPORTANT: When cancelling or adding more liquidity into an existing order, it's possible that fees
 * have been accrued. In those cases, the accrued fees are forfeited and added to the order, benefitting
 * the remaining limit order placers.
 *
...
abstract contract LimitOrderHook is BaseHook, IUnlockCallback {
    ...
    /// @dev Info for each order
    struct OrderInfo {
        /// @dev The currencies of the order.
        Currency currency0;
        Currency currency1;
        /// @dev The filled amounts for each currency.
        uint256 filledAmount0;
        uint256 filledAmount1;
        /// @dev The accrued fees for each currency.
        uint256 accruedFees0;
        uint256 accruedFees1;
        /// @dev Monotonic accumulators that accumulates the accrued fees per liquidity unit.
    ...
    /// @dev Info for each owner of the order.
    struct UserInfo {
        /// @dev Liquidity added by the owner.
        uint128 liquidity;
        /// @dev Checkpoints of the accumulated fees at the time of liquidity placement.
        /// Used to compute the portion of the accumulated fees that are owed to each owner.
    ...
    /// @dev Tracks the current order id for a given `orderKey`.
     ...
     * at the given `tickLower`, `zeroForOne` indicating the direction of the order, and `liquidity` the liquidity removed.
     */
    event Cancel(
        address indexed owner,
        OrderIdLibrary.OrderId indexed orderId,
    ...
    /// @dev Hooks into the `afterSwap` hook to get the ticks crossed by the swap and fill the orders that are crossed.
    ...
    /**
     * @dev Places a limit order by adding out of range liquidity at a specific tick. The order will be filled when the
     ...
     *
     * NOTE: If the placer adds more liquidity to an order he has placed before, any previous fees accrued are forfeited and
     * redistributed to the rest of the liquidity providers, updating their feeCheckpoints accordingly.
     */
    function placeOrder(PoolKey calldata key, int24 tickLower, bool zeroForOne, uint128 liquidity)
        public
        virtual
        onlyValidPools(key.hooks)
    {
        ...
        // NOTE: This must be done before updating the order state and re-baselining the user fee checkpoints.
        ...
        // Re-baseline the user fee checkpoints
    ...
    /**
     * @dev Cancels a limit order by removing it's liquidity from the pool. The interaction with the `poolManager` is done
     * via the `unlock` function, which will trigger the `{unlockCallback}` function.
     * NOTE: Partial cancellation is not supported, the entire liquidity added by the `msg.sender` will be removed.
     * NOTE: If the canceller is not the last liquidity provider, fees accrued during cancellation are kept in the order
     * and redistributed to remaining liquidity providers. If the caller removes the last liquidity of the order, any
     * accumulated accrued fees are paid out and the order id is reset to the default value.
     */
        ...
        onlyValidPools(key.hooks)
    {
        ...
        // subtract the liquidity from the order
        orderInfo.liquidityTotal -= liquidity;
        delete orderInfo.users[msg.sender];

        // Unlock the callback to the poolManager, the callback will trigger `unlockCallback` and remove the liquidity from the pool.
        ...
        if (removingAllLiquidity) {
            ...
            // @tbd the following resets may not be necessary as the order id is reset.
    ...
    /**
     * @dev Withdraws liquidity from a filled order. Can only be called after the order is filled.
     * Use `cancelOrder` to remove liquidity from unfilled orders. The interaction with the `poolManager` is done
     * via the `unlock` function, which will trigger the `{unlockCallback}` function.
     */
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        bool isLastWithdrawer = userInfo.liquidity == liquidityTotal;
        ...
        if (!isLastWithdrawer) {
            // Calculate the fill and fee amounts owed to the withdrawer depending on the provided liquidity.
            // NOTE: Rounds down, benefiting the rest of the liquidity providers and accumulating dust in the order.
            amount0Fill = FullMath.mulDiv(orderInfo.filledAmount0, userInfo.liquidity, liquidityTotal);
            amount1Fill = FullMath.mulDiv(orderInfo.filledAmount1, userInfo.liquidity, liquidityTotal);
            amount0Fee = ((userInfo.liquidity * orderInfo.accFee0PerLiqX128) - userInfo.feeCheckpoint0X128) >> 128;
            amount1Fee = ((userInfo.liquidity * orderInfo.accFee1PerLiqX128) - userInfo.feeCheckpoint1X128) >> 128;
        } else {
            // Include all the remaining accounting dust to the final withdrawer.
            amount0Fill = orderInfo.filledAmount0;
            amount1Fill = orderInfo.filledAmount1;
            amount0Fee = orderInfo.accruedFees0;
            amount1Fee = orderInfo.accruedFees1;
        }
        ...
        // Unlock callback to the poolManager. The callback will trigger `unlockCallback` and transfer withdrawn currency to `to`.
    ...
    /**
     * @dev Triggers the place callback to the poolManager and returns accrued fees from the placement.
     * NOTE: `tickLower` must be valid, i.e. within the range of `MIN_TICK` and `MAX_TICK`, defined in the `TickMath` library
    ...
    /**
     * @dev Triggers the cancel callback to the poolManager and returns accrued fees from cancellation.
     */
    ...
    /**
     * @dev Handles place order callbacks. Takes `placeData` containing the order details and adds the
     * specified liquidity to the pool out of range. Reverts if the order would be placed in range or on the wrong
     * side of the range. Any accrued fees in the position are taken and stored in the hook.
     */
        ...
        returns (uint256 amount0Fee, uint256 amount1Fee)
    {
        ...
        // Take any accrued fees from the pool to the hook.
        ...
        if (principalDelta.amount0() < 0) {
            ...
            if (principalDelta.amount1() != 0 || !placeData.zeroForOne) revert InvalidRange();
            ...
            if (principalDelta.amount0() != 0 || placeData.zeroForOne) revert InvalidRange();
    ...
    /**
     * @dev Handles cancel order callbacks. Removes liquidity from the pool and computes fee allocation.
     * NOTE: If not all the liquidity is removed, any accrued fees in the position are taken and stored in the hook.
     * Otherwise, if all the liquidity is removed, any previously accumulated accrued fees are transferred to the cancelling user.
     */
    function _handleCancelCallback(CancelCallbackData memory cancelData)
        ...
        returns (uint256 amount0Fee, uint256 amount1Fee)
    {
        // remove the liquidity from the pool. Any fees accrued by the position are included in the `callerDelta`
        (BalanceDelta callerDelta, BalanceDelta feesAccrued) = poolManager.modifyLiquidity(
            cancelData.key,
            ModifyLiquidityParams({
        ...
        // If not all liquidity is removed, any accrued fees in the position are taken and accumulated in the hook.
        if (!cancelData.removingAllLiquidity) {
            // Decrease the canceller payout by the accrued fees.
            ...
            // If all the liquidity is removed, the accrued fees are included to the cancelling user.
            principalDelta = callerDelta;

            // Also transfer any previously accumulated accrued fees (minted to hook by earlier cancellers)
        ...
        // Take the principal delta from the pool to the canceller.
        if (principalDelta.amount0() > 0) {
    ...
    /**
     * @dev Handles withdraw callbacks. Takes `withdrawData` containing withdrawal amounts and recipient,
    ...
    function _handleWithdrawCallback(WithdrawCallbackData memory withdrawData) internal virtual {
        // if the amount of currency0 is positive, burn the currency0 from the hook and pay the receiver
        ...
        // if the amount of currency1 is positive, burn the currency1 from the hook and pay the receiver
    ...
    /// @dev Retrieves the order for the given parameters, creating and initializing it if it does not exist yet.
     ...
     * and direction `zeroForOne`. Removes liquidity from the pool and stores it in the hook, updating the order state to track filled amounts.
     */
    function _fillOrder(PoolKey calldata key, int24 tickLower, bool zeroForOne) internal virtual {
        ...
        if (!orderId.equals(ORDER_ID_DEFAULT)) {
            ...
            // reset the order id to the default value
            _setOrderId(key, tickLower, zeroForOne, ORDER_ID_DEFAULT);
            ...
            if (callerDelta.amount0() > 0) {
                poolManager.mint(address(this), key.currency0.toId(), callerDelta.amount0().toUint256());
            }
            if (callerDelta.amount1() > 0) {
                poolManager.mint(address(this), key.currency1.toId(), callerDelta.amount1().toUint256());
            }

            // account the filled amounts.
            unchecked {
                orderInfo.filledAmount0 += principalDelta.amount0().toUint256();
                orderInfo.filledAmount1 += principalDelta.amount1().toUint256();
            }

            // account any accrued fees.
            _accountAccruedFees(orderInfo, feesAccrued.amount0().toUint256(), feesAccrued.amount1().toUint256());

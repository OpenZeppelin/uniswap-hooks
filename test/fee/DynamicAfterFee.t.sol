...
contract DynamicAfterFeeTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        dynamicFeesHook = DynamicAfterFeeMock(
            address(
                uint160(
        ...
        deployCodeTo(
            "test/mocks/DynamicAfterFeeMock.sol:DynamicAfterFeeMock",
            abi.encode(manager),
    ...
    function test_swap_100PercentLPFeeExactInput_succeeds() public {
        assertEq(
            BalanceDelta.unwrap(dynamicFeesHook.getTargetDelta(key.toId())),
            0
        );
    ...
    function test_swap_fuzz_succeeds(
        ...
        int128 targetAmount1 = deltaFee > 0
            ? int128(uint128(amountSpecified - deltaFee))
            : int128(0);

        if (zeroForOne) {
            dynamicFeesHook.setTargetDelta(
                key.toId(),
                toBalanceDelta(-int128(amountSpecified), targetAmount1)
            );
        } else {
            dynamicFeesHook.setTargetDelta(
                key.toId(),
                toBalanceDelta(targetAmount1, -int128(amountSpecified))
            );
        }

...
contract LimitOrderHookFuzzTest is HookTest {
    ...
    function testFuzz_placeOrder(int24 tickLower, bool zeroForOne, uint128 liquidity) public {
        ...
        if (isValidRange) {
            ...
            uint256 lpOrderLiquidity = hook.getUserInfo(OrderIdLibrary.OrderId.wrap(1), address(lp1)).liquidity;
            ...
            assertEq(
                hook.getUserInfo(OrderIdLibrary.OrderId.wrap(0), address(lp1)).liquidity,
                0,
                "order should have 0 liquidity"
            );
        }
    }
    ...
    function testFuzz_feesAccrued_unfilled(int24 tickLower, bool zeroForOne, uint128 liquidity) public {
...

        {
            ...
            assertEq(
                hook.getUserInfo(OrderIdLibrary.OrderId.wrap(1), address(lp1)).liquidity,
                liquidity,
                "lp1 owns liquidity"
            );
            assertEq(
                hook.getUserInfo(OrderIdLibrary.OrderId.wrap(1), address(lp2)).liquidity,
                liquidity,
                "lp2 owns liquidity"
            );
